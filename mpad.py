...
tool_bar.pack(side=tk.TOP, fill=tk.X)  ## horizontal = x, vertical = y, or both 
...
def changed(event=None):
    ...
    if text_editor.edit_modified():
        text_changed = True
        words = len(text_editor.get(1.0, 'end-1c').split()) # count words
        charactors = len(text_editor.get(1.0, 'end-1c').replace(' ', '')) # count characters
        status_bar.config(text=f'Characters : {charactors} Words: {words}')
...
file.add_command(label='Save as', image=save_as_icon, compound=tk.LEFT, accelerator='Ctrl+M', command=save_as)
...
main_application.bind('<Control-m>', save_as)
